...
# 公信宝报告链接爬去
def from_url_gxb(url, data, targetPath):
    # w文件命名规则
    year = datetime.datetime.now().strftime('%Y')
    month = datetime.datetime.now().strftime('%m')
    day = datetime.datetime.now().strftime('%d')
    file_path = file_target_path + os.path.sep + year + os.path.sep + month + os.path.sep + day + os.path.sep + data[
        "apply_no"] + os.path.sep + 'gxb'
    pdf_file_name = data['apply_no'] + '_gxb_' + str(data["file_seq_no"]) + '.pdf'

    # 报告输出位置
    file_path_sql = file_target_path + "/" + year + "/" + month + "/" + day + "/" + data["apply_no"] + "/" + 'gxb'
...
def crew_gxb(url, data, targetPath):
    from_url_gxb(url, data, targetPath)