...
def from_url_spb(url, data, target_path):
    logger = logging.getLogger('root')

    pdf_file_name = data['apply_no'] + '_spb_100.pdf'
    ...
    file_path = target_path + os.path.sep + year + os.path.sep + month + os.path.sep + day + os.path.sep + data[
        "apply_no"] + os.path.sep + 'spb'

    # 报告输出位置
    file_path_sql = target_path + "/" + year + "/" + month + "/" + day + "/" + data["apply_no"] + "/" + 'spb'
...
# 百融爬虫
def crew_spb(url, data, target_path):
    # 调用爬虫程序
    from_url_spb(url, data, target_path)
